...
class PhoneManufacturer(ABC):
    ...
    def produce_phone(self) -> Phone:
        p_factory = AbstractPhoneFactory.get_factory(self.client_options["phone"])
        if p_factory.check_model(self.client_options["model"]) is ValueError:
            return None
        screen, cpu, camera = self._request_parts(p_factory)
        return Phone(model=self.client_options["model"], screen=screen, cpu=cpu, camera=camera)

    def _request_parts(self, p_factory: Any) -> Any:
        screen = p_factory.produce_screen(self.client_options["resolution"])
        cpu = p_factory.produce_cpu(self.client_options["cpu"])
        camera = p_factory.produce_camera(self.client_options["camera"])

        return screen, cpu, camera
...
client = Client()
p1 = client.request_phone(phone_specs1)
# p2 = client.request_phone(phone_specs2)
# p3 = client.request_phone(phone_specs3)
